...
namespace MVCCore.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        public static IEngine CreateEngineInstance(MVCCoreConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'MVCCore.Infrastructure.IEngine' and cannot be configured in /configuration/nop/engine[@engineType] for that purpose.");
                return Activator.CreateInstance(engineType) as IEngine;
            }

            return new MvcEngine();
        }
